...
include get_stylesheet_directory() . '/inc/class-block-editor.php';
...
class Automatically_Paginate_Posts {
	...
	public function settings_field_num_pages() {
		$num_pages = 20;
		$max_pages = apply_filters( 'autopaging_max_num_pages', 100 );
	...
	public function sanitize_num_pages( $num_pages ) {
		return max( 20, min( intval( $num_pages ), apply_filters( 'autopaging_max_num_pages', 100 ) ) );
	}
	...
	public function sanitize_num_words( $num_words ) {
		...
		return max( $num_words, apply_filters( 'autopaging_min_num_words', 1500 ) );
	}

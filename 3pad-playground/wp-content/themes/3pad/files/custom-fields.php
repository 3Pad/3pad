...
function telegram_comments_home()
{
    ...
    $site_path = parse_url($site_url, PHP_URL_PATH) ?? '';  // Ensuring $site_path is not null
    $site_path = ltrim($site_path, '/');  // Remove leading slashes if any
...
function show_comment_popup()
{
    ...
    // Iframe Content
    // Define the mapping of content checks to ACF fields and other related variables
    $content_checks = [
        'embed1-content'  => 'enable_iframe_content_1',
        'embed2-content'  => 'enable_iframe_content_2',
        'embed3-content'  => 'enable_iframe_content_3',
        'embed4-content'  => 'enable_iframe_content_4',
        'embed5-content'  => 'enable_iframe_content_5',
        'embed6-content'  => 'enable_iframe_content_6',
        'embed7-content'  => 'enable_iframe_content_7',
        'embed8-content'  => 'enable_iframe_content_8',
        'embed9-content'  => 'enable_iframe_content_9',
        'embed10-content' => 'enable_iframe_content_10',
    ];

    // Loop through the content checks to process iframe contents
    foreach ($content_checks as $content_check => $field_name) {
        // Dynamically construct variable names
        $content_variable = 'iframe_content' . substr($content_check, 5, 1);
        $style_variable   = 'style' . substr($content_check, 5, 1);
        $doc_variable     = 'doc' . substr($content_check, 5, 1);

        // Check if the current content check matches the expected value
        if (${"content_check" . substr($content_check, 5, 1)} == $content_check) {
            // Get the iframe content from ACF
            ${$content_variable} = get_field($field_name, $settings_page);

            // Clean the iframe content using preg_replace to remove line breaks and escape single quotes
            ${$content_variable} = preg_replace("/[\r\n]+/", '', ${$content_variable});  // Remove line breaks
            ${$content_variable} = preg_replace("/'/", "\\'", ${$content_variable});  // Escape single quotes

            // Define the style for the iframe
            ${$style_variable} = '<style>iframe { border: none; } body { color: white; padding: 2% 8% 0% 8%; } * { overflow: scroll; max-width: 100%; }</style>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed1" loading="lazy" src="' . $srcdoc1 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed2" loading="lazy" src="' . $srcdoc2 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed3" loading="lazy" src="' . $srcdoc3 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed4" loading="lazy" src="' . $srcdoc4 . '"  ></iframe>
</div>
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed5" loading="lazy" src="' . $srcdoc5 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed6" loading="lazy" src="' . $srcdoc6 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed7" loading="lazy" src="' . $srcdoc7 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
    ...
    if ($content_embed8 != NULL || $content_check8 == 'embed8-content') {
        ...
        <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed8" loading="lazy" src="' . $srcdoc8 . '"  ></iframe>
    </div>
            </li>  
            </ul>
          </div>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed9" loading="lazy" src="' . $srcdoc9 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
    ...
    <iframe webkitallowfullscreen mozallowfullscreen allowfullscreen class="iframe_style" id="embed10" loading="lazy" src="' . $srcdoc10 . '"  ></iframe>
</div>
        </li>  
        </ul>
      </div>';
...
add_action('wp_footer', 'meta_db', 9999);
